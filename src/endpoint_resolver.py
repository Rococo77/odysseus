...
def resolve_endpoint(
    ...
    model = (
        get_user_setting(
            f"{setting_prefix}_model", owner or "", settings.get(f"{setting_prefix}_model", "")
        )
        or ""
    ).strip()
    ...
    if setting_prefix == "utility" and not ep_id:
        ep_id = (
            get_user_setting(
                "default_endpoint_id", owner or "", settings.get("default_endpoint_id", "")
            )
            or ""
        ).strip()
        model = (
            get_user_setting("default_model", owner or "", settings.get("default_model", "")) or ""
        ).strip()
    ...
    if not ep_id and setting_prefix != "utility":
        ...
        model = (
            get_user_setting("utility_model", owner or "", settings.get("utility_model", "")) or ""
        ).strip()
        if not ep_id:
            ep_id = (
                get_user_setting(
            ...
            model = (
                get_user_setting("default_model", owner or "", settings.get("default_model", ""))
                or ""
            ).strip()
