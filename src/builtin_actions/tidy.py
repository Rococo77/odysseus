"""Tidy/cleanup automation actions.
...
async def action_consolidate_memory(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
        ...
        kept_all = [m for m in all_memories if not _belongs_to_owner(m) or m.get("id") in keep_ids]
        if owner:
            for mem in kept_all:
        ...
        extra = (
            f" (+{removed - len(removed_examples)} more)" if removed > len(removed_examples) else ""
        )
