...
class TaskScheduler:
            ...
            try:
                for label, start, end in [
                    ...
                    for tier in ("critical", "high", "normal", "low"):
                        ...
                        if not items:
                            continue
                        marker = {"critical": "[!!]", "high": "[!]", "normal": "  ", "low": " ·"}[
                            tier
                        ]
                        for ev in items:
    ...
    async def _execute_llm_task(self, task, db) -> str:
        ...
        try:
            if tz_name:
                ...
                now_local = (
                    datetime.utcnow().replace(tzinfo=timezone.utc).astimezone(ZoneInfo(tz_name))
                )
    ...
    async def _execute_research_task(self, task, db) -> str:
        ...
        try:
            ...
            eps = db2.query(ModelEndpoint).filter(ModelEndpoint.is_enabled == True).all()
            for ep in eps:
                if normalize_base(ep.base_url) in endpoint_url or endpoint_url in normalize_base(
                    ep.base_url
                ):
                    headers = build_headers(ep.api_key, normalize_base(ep.base_url))
                    break
