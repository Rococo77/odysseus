...
async def _auto_summarize_pass_single(days_back: int = 1, account_id: str | None = None) -> str:
    ...
    try:
        ...
        _sum_existing = {
            r[0] for r in _c.execute("SELECT message_id FROM email_summaries").fetchall()
        }
        _reply_existing = {
            r[0] for r in _c.execute("SELECT message_id FROM email_ai_replies").fetchall()
        }
        _tag_existing = (
        ...
        _cal_existing = (
            {
                r[0]
                for r in _c.execute("SELECT message_id FROM email_calendar_extractions").fetchall()
            }
            if auto_cal
            else set()
        )
        ...
        auto_urgent = False
        _urgent_existing = (
            {r[0] for r in _c.execute("SELECT message_id FROM email_urgency_alerts").fetchall()}
            if auto_urgent
            else set()
        )
        _c.close()
        ...
        try:
            _self_self_addr = (
                (_get_email_config(account_id).get("from_address") or "").strip().lower()
            )
        except Exception:
            _self_self_addr = ""
        ...
        for _entry in uid_list:
            ...
            try:
                ...
                _is_alert_echo = bool(
                    re.match(r"^\s*(\[(HIGH|CRITICAL|MEDIUM|LOW)\]\s*)+", _subj_raw, re.IGNORECASE)
                )
                ...
                if need_cal:
                    ...
                    try:
                        ...
                        cal_extract = re.sub(
                            r"^```(?:json)?\s*|\s*```$", "", cal_extract, flags=re.MULTILINE
                        ).strip()
                                    ...
                                    for op in ops[:3]:
                                        ...
                                        if action == "cancel":
                                            ...
                                            r = await do_manage_calendar(
                                                json.dumps({"action": "delete_event", "uid": cuid})
                                            )
                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    open_url = (
                                        f"{_pub}/#email={_urlquote(_folder, safe='')}:{uid_str}"
                                        if _pub
                                        else ""
                                    )
    ...
    try:
        ...
        for r in rows:
            ...
            try:
                ...
                conn2 = sqlite3.connect(SCHEDULED_DB)
                conn2.execute(
                    "UPDATE scheduled_emails SET status='failed', error=? WHERE id=?", (str(e), sid)
                )
                conn2.commit()
                conn2.close()
