...
# Email-HTML helpers live in their own module.
from routes.email_html import _md_to_email_html, _sanitize_email_html
            ...
            if filter_ == "unread":
                ...
                data = [
                    b" ".join(
                        sorted(
                            _uids,
                            key=lambda x: int(x) if str(x, "ascii", "ignore").isdigit() else 0,
                        )
                    )
                ]
                status = "OK"
            ...
            if uid_list:
                ...
                try:
                    ...
                    for _, raw_header in grouped:
                        if not raw_header:
                            continue
                        mid = (
                            email_mod.message_from_bytes(raw_header).get("Message-ID", "") or ""
                        ).strip()
                        if mid:
                ...
                for meta_b, raw_header in grouped:
                    try:
                        ...
                        tag_entry = _tag_by_message_id.get(message_id.strip()) or _tag_by_uid.get(
                            uid_num, {}
                        )
                        emails.append(
                            {
                ...
                for uid in uid_list:
                    try:
                        ...
                        for part in msg_data:
                            if isinstance(part, tuple):
                                meta = (
                                    part[0].decode() if isinstance(part[0], bytes) else str(part[0])
                                )
                                if (
                        ...
                        for part in msg_data:
                            if isinstance(part, tuple):
                                meta_b = (
                                    part[0] if isinstance(part[0], bytes) else str(part[0]).encode()
                                )
            ...
            if mark_seen:
                try:
                    _asyncio.create_task(
                        _asyncio.to_thread(_mark_email_seen_sync, uid, folder, account_id, owner)
                    )
                except RuntimeError:
                    pass
            return cached
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(
                            _search_uids(
                                conn, f"(HEADER X-Odysseus-Kind {_search_quote('reminder')})"
                            )
                        )
                        uids.update(
                            _search_uids(conn, f"(SUBJECT {_search_quote('Reminder (Odysseus):')})")
                        )
                        for addr in own_addrs:
                            ...
                            uids.update(
                                _search_uids(
                                    conn, f"(FROM {addr_q} SUBJECT {_search_quote('Reminder:')})"
                                )
                            )
                        if not uids:
                            continue
                        ...
                        for uid in reversed(uids):
                            try:
                                ...
                                if st2 != "OK":
                                    continue
                                raw = (
                                    msg_data[0][1] if msg_data[0] and len(msg_data[0]) > 1 else b""
                                )
                                ...
                                for field in ["From", "To", "Cc"]:
                                    ...
                                    for part in val.split(","):
                                        ...
                                        if name.lower() in part.lower():
                                            ...
                                            if addr and "@" in addr:
                                                display = (
                                                    part.split("<")[0].strip().strip('"') or addr
                                                )
                    ...
                    with _imap(_account_id, owner=owner) as imap:
                            ...
                            try:
                                ...
                                if st_sel == "OK":
                                    ...
                                    if st_uid == "OK" and uid_data and uid_data[0]:
                                        sent_uid = (
                                            uid_data[0].split()[-1].decode("ascii", errors="ignore")
                                        )
                            except Exception:
                                pass
                        ...
                        if _in_reply_to:
                            try:
                                ...
                                mid = (
                                    _in_reply_to.strip().lstrip("<").rstrip(">").replace('"', '\\"')
                                )
    ...
    async def ai_reply(data: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            try:
                ...
                _attempted = (
                    ", ".join(
                        f"{m}@{u.split('/')[2] if '/' in u else u}" for u, m, _ in _candidates
                    )
                    or "no candidates"
                )
                return {
